...
if (isset($_GET["deleted_id"])) {
    ...
    if ($stmt_del->execute()) {
        echo "<script>alert('Order has been deleted successfully'); window.location.href='Order.php';</script>";
    } else {
                    ...
                    <div class="card-group">
                        ...
                        if ($result->num_rows > 0) {
                            while ($row = $result->fetch_assoc()) {
                                echo "<div class='card'>";
                                if ($row["image"]) {
                                    echo '<img src="uploads/' . $row["image"] . '" class="card-img-top" alt="Product Image" style="max-height: 200px; object-fit: cover;">';
                                } else {
